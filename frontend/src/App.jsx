      ...
      <section className="section direct-add">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn" onClick={addDirect}>追加</button>
          </div>
        </div>
      </section>
              ...
              <li key={todo.id} className={todo.completed ? 'completed' : ''}>
                {editId === todo.id ? (
                  ...
                  <div className="row" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span className="meta-id" style={{ minWidth: 32 }}>#{todo.id}</span>
                    <div className="title" style={{ flex: 1, cursor: 'pointer' }} onClick={() => startEdit(todo)}>
                      {todo.title} {todo.due_date && <span className="due">({todo.due_date})</span>}
                    </div>
                    {!todo.completed && (
                    ...
                    <span className={`meta-status ${todo.completed ? 'done' : 'pending'}`} style={{ minWidth: 56 }}>{todo.completed ? '完了' : '未完了'}</span>
                  </div>
                )}
              </li>
            ))}
